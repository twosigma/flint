...
class ClockSpec extends FlatSpec with SharedSparkContext {
  ...
  "Clock" should "generate clock ticks correctly" in {
    var clock = Clock(1000L, 2000L, 300L, 0L).toArray
    var benchmark = Array(1000L, 1300L, 1600L, 1900L)
    assert(clock.deep == benchmark.deep)

    clock = Clock(1000L, 2000L, 300L, 500L).toArray
    benchmark = Array(1200L, 1500L, 1800L)
    assert(clock.deep == benchmark.deep)
  }

  it should "generate clock ticks in RDD correctly" in {
    var clockRdd = Clock(sc, 1000L, 5000L, 30, 0L, 10).map(_._1)
    var clockStream = Clock(1000L, 5000L, 30, 0L)
    assert(clockRdd.collect().deep == clockStream.toArray.deep)

    clockRdd = Clock(sc, 1000L, 5000L, 30, 500L, 10).map(_._1)
    clockStream = Clock(1000L, 5000L, 30, 500L)
    assert(clockRdd.collect().deep == clockStream.toArray.deep)
  }

  it should "generate clock ticks in TimeSeriesRDD correctly" in {
    val clockTSRdd: TimeSeriesRDD = Clock(sc, "5h", "0d", "20010101", "20010201")
    assert(clockTSRdd.schema == Schema())
    val clockStream = Clock(
  ...
  it should "generate clock ticks with offset in TimeSeriesRDD correctly" in {
    val clockTSRdd1: TimeSeriesRDD = Clock(sc, "1d", "8h", "20010101", "20010201")
    val clockTSRdd2: TimeSeriesRDD = Clock(sc, "1d", "0h", "20010101 08:00", "20010201")
  ...
  it should "generate clock ticks with offset & time zone in TimeSeriesRDD correctly" in {
    val clockTSRdd1: TimeSeriesRDD = Clock(sc, "1d", "8h", "20010101", "20010201", "UTC")
    val clockTSRdd2: TimeSeriesRDD = Clock(sc, "1d", "0h", "20010101 08:00 +0000", "20010201 00:00 +0000", "MST")
    assert(clockTSRdd1.collect().deep == clockTSRdd2.collect().deep)
  }

  it should "generate clock ticks with default in TimeSeriesRDD correctly" in {
    val clockTSRdd1: TimeSeriesRDD = Clock(sc, "1d", "0h", "19900101", "20300101", "UTC")
    val clockTSRdd2: TimeSeriesRDD = Clock(sc, "1d")
    assert(clockTSRdd1.collect().deep == clockTSRdd2.collect().deep)
  }
}
