...
class NthMomentSummarizerSpec extends FlatSpec with SharedSparkContext {
  ...
  "NthMomentSummarizer" should "`computeNthMoment` correctly" in {
    ...
    var results = priceTSRdd.summarize(Summary.nthMoment("price", 0), Seq("tid")).collect()
    ...
    results = priceTSRdd.summarize(Summary.nthMoment("price", 1), Seq("tid")).collect()
    ...
    results = priceTSRdd.summarize(Summary.nthMoment("price", 2), Seq("tid")).collect()
    ...
    results = priceTSRdd.summarize(Summary.nthMoment("price", 3), Seq("tid")).collect()
    ...
    results = priceTSRdd.summarize(Summary.nthMoment("price", 4), Seq("tid")).collect()
  ...
  it should "`computeNthCentralMoment` correctly" in {
    val priceTSRdd = from("Price.csv", Schema("tid" -> IntegerType, "price" -> DoubleType))
    var results = priceTSRdd.summarize(Summary.nthMoment("price", 0), Seq("tid")).collect()
    results = priceTSRdd.summarize(Summary.nthCentralMoment("price", 1), Seq("tid")).collect()
    ...
    results = priceTSRdd.summarize(Summary.nthCentralMoment("price", 2), Seq("tid")).collect()
    ...
    results = priceTSRdd.summarize(Summary.nthCentralMoment("price", 3), Seq("tid")).collect()
    ...
    results = priceTSRdd.summarize(Summary.nthCentralMoment("price", 4), Seq("tid")).collect()
