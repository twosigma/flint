...
class CovarianceSummarizerSpec extends FlatSpec with SharedSparkContext {
  ...
  "CovarianceSummarizer" should "`computeCovariance` correctly" in {
    ...
    var results = input.summarize(Summary.covariance("price", "price2"), Seq("id")).collect()
    ...
    results = input.summarize(Summary.covariance("price", "price3"), Seq("id")).collect()
    ...
    results = input.summarize(Summary.covariance("price", "price4"), Seq("id")).collect()
    ...
    results = input.summarize(Summary.covariance("price", "price5"), Seq("id")).collect()
    ...
    results = input.summarize(Summary.covariance("price", "forecast"), Seq("id")).collect()
