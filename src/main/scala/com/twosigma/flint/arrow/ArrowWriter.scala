...
private[arrow] class StringWriter(val valueVector: NullableVarCharVector) extends ArrowFieldWriter {
  ...
  override def setValue(input: SpecializedGetters, ordinal: Int): Unit = {
    ...
    valueMutator.setSafe(count, utf8.getBytes, 0, utf8.numBytes())
  }
}

private[arrow] class BinaryWriter(
  val valueVector: NullableVarBinaryVector
...
private[arrow] class DateWriter(
  val valueVector: NullableDateDayVector
) extends ArrowFieldWriter {
...
private[arrow] class TimestampWriter(
  val valueVector: NullableTimeStampMicroTZVector
) extends ArrowFieldWriter {
